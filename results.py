...
class Get_Result():
    ...
    def get_image_pre_all(self, image_dir):
        img = cv2.imread(image_dir)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (224, 224))
        ...
        return self.get_image_all("Base.png")
        
    def get_image_all(self, image_dir):
        img = cv2.imread(image_dir)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (224, 224))
        ...
        self.pred = self.model.predict(img_array, verbose=0)
        return self.get_results_all()
        
    def get_results_all(self):
        probs = self.pred[0]
        results_dict = {}
        for i, cls in enumerate(self.class_names):
            results_dict[cls] = float(probs[i] * 100)
        return results_dict
